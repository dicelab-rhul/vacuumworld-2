...
public class Grid implements Serializable {
     ...
     */
    public Position moveAgent(VacuumWorldAgentAppearance agent, Position position) {
	...
	return position.getX() < 0 || position.getY() < 0 || position.getX() >= dimension
     ...
     * @param position
     *            to check
     * @return true if the {@link Dirt} is at the {@link Position}, false otherwise
     */
     ...
     * @param position
     *            to check
     * @return true if the {@link Agent} is at the {@link Position}, false otherwise
     */
    public boolean containsAgent(VacuumWorldAgentAppearance agent, Position position) {
	return agent.getPosition().equals(position);
    }

    /**
     * Places the given {@link Dirt} at the given {@link Position} if it does not
     * already contain {@link Dirt}.
     * 
     * @param dirt
     *            to place
     * @param position
    ...
    /**
     * Places the given {@link Agent} at the given {@link Position} if it does not
     * already contain an {@link Agent}. This method should only be called when
     * adding a new {@link Agent}, to move existing {@link Agent}s the
     ...
     * @param position
     *            to place at
     * @return true if the {@link Agent} was successfully placed, false otherwise
     */
    public boolean placeAgent(VacuumWorldAgent agent, Position position, Orientation orientation) {
	VacuumWorldTile container = gridmap.get(position);
     ...
     *            to place
     */
    public void fillRandom(Collection<AbstractAutonomousAgent> agents, Collection<PassiveBody> dirts) {
	Random random = new Random();
	agents.forEach(a -> {
	    placeAgent((VacuumWorldAgent) a, new Position(random.nextInt(dimension), random.nextInt(dimension)),
		    RandomUtility.getRandomEnum(Orientation.class));
	    ((VacuumWorldAgent) a).getAppearance().setColor(RandomUtility.getRandomEnum(BodyColor.class));
	});
	dirts.forEach(d -> placeDirt((Dirt) d, new Position(random.nextInt(dimension), random.nextInt(dimension))));
    }

    /**
     * Fills this {@link Grid} with the given {@link Agent}s and {@link Dirt}s. The
     * {@link Position}, {@link Orientation} and {@link Color} should have be
     * defined in their {@link VacuumWorldAgentAppearance}.
     * 
     * @param agents
     *            to place
     * @param dirts
     *            to place
     */
    public void fillGrid(Collection<AbstractAutonomousAgent> agents, Collection<PassiveBody> dirts,
	    Collection<AbstractAvatarAgent<?>> avatars) {
	agents.forEach(a -> {
	    VacuumWorldAgentAppearance ap = (VacuumWorldAgentAppearance) a.getAppearance();
	...
	gridmap.keySet().forEach(pos -> {
	    int start = ((pos.getY() * this.dimension + pos.getX()) * 3) + pos.getY() * 2;
	    int end = start + 3;
	    builder.replace(start, end, "[" + gridmap.get(pos).toString().substring(0, 1) + "]");
	});
