...
public class GridPanel extends JPanel {
	...
	return new Dimension(this.getWidth() / gridDimension, this.getHeight() / gridDimension);
    }

    public Point snapSelectionBox(Point position) {
	if (position.getY() >= this.getHeight() - 4 || position.getX() >= this.getWidth()) {
	    return new Point(-Integer.MAX_VALUE, -Integer.MAX_VALUE);
	}
	double sdx = (double) this.getWidth() / gridDimension;
	double sdy = (double) this.getHeight() / gridDimension;

	return new Point((int) (position.x - position.x % sdx), (int) (position.y - position.y % sdy));
    }
	...
	return !(position.getX() < 0 || position.getX() >= gridDimension || position.getY() < 0
		|| position.getY() >= gridDimension);
    }
    ...
    protected void drawAgent(Graphics g, Position position, BodyColor color, Orientation orientation) {
	BufferedImage img = VacuumWorldView.agentimages.get(color).get(orientation);
	drawBody(g, img, position);
    }
	...
	g.drawImage(img, (int) (position.getX() * incw), (int) (position.getY() * inch),
		(int) ((position.getX() + 1) * incw), (int) ((position.getY() + 1) * inch), 0, 0, img.getWidth() - 2,
