...
public class ClassFinder {
    ...
    public static Collection<String> getClassNamesFromJar(File jar) throws ClassFinderException {
	Collection<String> names = new HashSet<>();
	ZipInputStream zip = null;
	try (FileInputStream fins = new FileInputStream(jar)) {
	    zip = new ZipInputStream(fins);
	    for (ZipEntry entry = zip.getNextEntry(); entry != null; entry = zip.getNextEntry()) {
		if (!entry.isDirectory() && entry.getName().endsWith(CLASSEXTENSION)) {
		    String name = entry.getName();
		    name = name.substring(0, name.length() - CLASSEXTENSION.length());
    ...
    public static Collection<Class<?>> getAnnotatedClasses(Class<? extends Annotation> annotation,
    ...
    public static Collection<Class<?>> getSubClasses(Class<?> superclass, Collection<Class<?>> classes) {
	...
	for (File f : classfiles) {
	    classnames.add(f.getAbsolutePath().replace(root.getAbsolutePath() + "\\", "").replace(CLASSEXTENSION, "")
		    .replace("\\", "."));
	}
	return classnames;
    }
    ...
    /**
     * The {@link Exception} that is thrown by methods in the {@link ClassFinder}
     * class for any reason that results in the failure to produce desired output.
     * i.e. to properly find any classes.
     * 
     * @author Ben Wilkins
     *
     */
