...
public class RotateButton extends CustomButton {
    ...
    private static final BufferedImage IMG = ImageUtilities.getOverlayedImage(
	    VacuumWorldView.IMGLOADER.loadImage(VacuumWorldView.IMGPATH + "rotate_button" + VacuumWorldView.EXTENSION),
	    Color.BLACK, 1f);
    private static final BufferedImage ROTATEHOVER = ImageUtilities.getOverlayedImage(IMG, Color.WHITE, 0.3f);
	...
	if (left) {
	    ...
	    g2.drawImage(image, 0 + this.getWidth(), 0, -this.getWidth(), this.getHeight(), null);
	}
    }
	...
	if (this.current != null) {
	    this.current.setOrientation(this.current.getOrientation().getLeft());
	    this.left = true;
	}
    }
	...
	if (this.current != null) {
	    this.current.setOrientation(this.current.getOrientation().getRight());
	    this.left = false;
	}
    }
